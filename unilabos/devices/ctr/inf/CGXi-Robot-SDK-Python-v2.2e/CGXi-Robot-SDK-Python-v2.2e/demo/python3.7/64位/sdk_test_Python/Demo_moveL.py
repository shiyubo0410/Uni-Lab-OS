...
#直线运动——cr_moveL
def api_demo_cr_moveL():
    pointControlPara = basestruct.PointControlPara()
    speed = 100
    acc = 250
    jointpose = (60, 60, -60, 60, 60, 60)
    pointControlPara.jointpos = jointpose
    ...
    pointControlPara.poseTranType = 0
    pointControlPara.motiontriggerMode = 1
    pointControlPara.pose = (0, -438, 887, -57.34, 0, -149)
...
def api_demo_cr_move_line():
    ...
    isBlock = 0
    result = cgxiapi.cr_move_line(robotHandle, pointControlPara, isBlock)
    print("返回结果:", result.value)
    assert (result.value == 0)
