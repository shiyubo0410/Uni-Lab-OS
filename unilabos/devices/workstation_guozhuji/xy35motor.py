...
class XY35Motor:
    ...
    def __init__(self, address: int = 1, port: str = None, baudrate: int = 9600):
        ...
        Args:
            address (int): Modbus地址
            port (str): 串口路径或设备ID引用(如 "serial_pump")
            baudrate (int): 波特率
        """
        ...
        if port and isinstance(port, str) and not port.startswith("COM") and not port.startswith("/dev/"):
            # port 是设备ID（如 "serial_pump"），设置为属性供工作站识别
            self.hardware_interface = port  # 字符串，工作站会替换为实际串口对象
            print(f"[XY35Motor 地址{address}] 等待工作站注入串口接口: {port}")
        else:
            self.hardware_interface = None
            if port:
                print(f"[XY35Motor 地址{address}] 直接串口模式: {port}")
