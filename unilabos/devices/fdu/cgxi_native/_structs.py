# -*- coding: utf-8 -*-
"""CGXi 机器人 ctypes 结构体定义。

本文件**一字不改**地从厂家 Python 3.7 SDK 的 ``basestruct.py`` 拷贝而来::

    unilabos/devices/ctr/inf/CGXi-Robot-SDK-Python-v2.2e/
        CGXi-Robot-SDK-Python-v2.2e/demo/python3.7/64位/
        sdk_test_Python/basestruct.py

因为这是 **纯 ctypes 定义**（``Structure`` + ``c_int`` 等），和 Python 版本无关，
所以可以直接跨 Python 3.7 / 3.11 / 3.12 使用。未来若厂家出 Python 3.11 版 SDK，
可直接对比本文件与新 basestruct.py 的差异来升级。

原始文件头的 ``#!/usr/bin/python3`` 被替换为本模块 docstring；其余内容一字不改。
"""
import sys
import types
