        ...
        static void Main(string[] args)
        {
            ...
            if (result == CRresult.sucess)
            {
                ...
                if (result == CRresult.sucess)
                {
                    ...
                    {
                        result = CS.cr_kineForward(robotHandle, pointcsnode.point.toolAxisAngle, pointcsnode.point.toolPosition);
                        if (result == CRresult.sucess)
                        {
                            result = CS.cr_compute_cs_point(pointcsnode.point.toolPosition, 6, pointcsnode.point.toolPosition, 6);
                            if (result == CRresult.sucess)
                    ...
                    int index = 0;
                    result = CS.cr_cfg_cs_point_get(robotHandle, index, ref pointcsnode);
                    //读取索引号为0的点坐标系数据
                    if (result == CRresult.sucess)
                    {
                        result = CS.cr_cfg_cs_point_delete(robotHandle, index);
                        //删除索引号为0的点坐标系
                        if (result == CRresult.sucess)
                        {
                            Console.WriteLine($"行号：{GetLineNum().ToString()},已成功删除点坐标系");
                        }
                        else
                        {
                            Console.WriteLine($"行号：{GetLineNum().ToString()},结果：{result}");
