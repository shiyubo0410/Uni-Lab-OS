...
class XRDClient:
    def __init__(self, host='127.0.0.1', port=6001, timeout=10.0, debug=True):
        """
        初始化XRD D7-Mate客户端
    ...
    def connect(self):
        ...
        # Debug模式：跳过实际连接
        if self.debug:
            print("[DEBUG] 模拟设备连接成功")
            self.sock = True  # 设置为非None值表示"已连接"
            return
    ...
    def start_auto_mode(self, status: bool) -> dict:
        ...
        # Debug模式：返回模拟响应
        if self.debug:
            print(f"[DEBUG] 模拟启动自动模式: status={status}")
            return {
                "status": True,
                "timestamp": time.time(),
                "message": "自动模式已启动（模拟）"
    ...
    def send_sample_ready(self, sample_id: str, start_theta: float, end_theta: float, 
        ...
        # Debug模式：返回模拟响应
        if self.debug:
            print(f"[DEBUG] 模拟样品准备完成: sample_id={sample_id}, start_theta={start_theta}, "
                  f"end_theta={end_theta}, increment={increment}, exp_time={exp_time}")
            return {
                "status": True,
                "timestamp": time.time(),
                "message": "样品准备完成（模拟）"
    ...
    def get_current_acquire_data(self) -> dict:
        ...
        # Debug模式：返回模拟响应
        if self.debug:
            print("[DEBUG] 模拟获取采集数据")
            return {
                "status": True,
                "timestamp": time.time(),
                "sample_id": "debug_sample",
                "Energy": [10.0, 15.0, 20.0, 25.0],
    ...
    def sample_down(self, sample_station: int) -> dict:
        ...
        # Debug模式：返回模拟响应
        if self.debug:
            print(f"[DEBUG] 模拟下样请求: 工位={sample_station}")
            return {
                "status": True,
                "timestamp": time.time(),
                "sample_info": {
    ...
    def start(self, sample_id: str = "", start_theta: float = 10.0, end_theta: float = 80.0,
                 ...
                 string: str = "", upload_to_oss: bool = True) -> dict:
        """
        Start 主流程：
        1) 启动自动模式；
        2) 发送上样请求并等待允许；
        ...
        try:
          # 强制类型转换：除 sample_id 外的所有输入均转换为 float（若为字符串）
            ...
            if isinstance(wait_minutes, str):
                wait_minutes = _to_float(wait_minutes, 0.5)
            ...
            if down_station_idx is None:
                raise ValueError("未检测到任一下样位 True，流程未完成")
            ...
            # 拿到实际测量的结果文件
            example_sample = str(uuid.uuid4())
            samples_to_result = []
            
            if self.debug:
                # Debug模式：使用示例文件
                target_file_path = Path(__file__).parent / "get_sample_down_202511111259xrdtest_2025-11-11_14-27-00.csv"
                oss_upload_result = oss_upload(target_file_path, driver_name="XRD_d7", exp_type="Normal")
                
                if oss_upload_result["success"]:
                    samples_to_result.append({
                        "sample_uuid": example_sample,
                        "oss_path": oss_upload_result["oss_path"],
                        "extra": {
                            "original_path": oss_upload_result["original_path"],
                        }
                    })

            return {
                "unilabos_samples": samples_to_result
            }
                
            #     if upload_to_oss:
            #         # 使用正确的参数调用 oss_upload
            #         oss_path = "/XRD_D7/results"
            #         upload_success = oss_upload(
            #             file_path=str(target_file_path),
            #             oss_path=oss_path,
            #             filename=None,
            #             process_key="XRD-D7-result",
            #             device_id="XRD_D7MATE_STATION"
            #         )
                    
            #         if upload_success:
            #             # 构造完整的OSS路径
            #             oss_full_path = f"{oss_path}/{target_file_path.name}"
            #             samples_to_result.append({
            #                 "sample_uuid": example_sample,
            #                 "oss_path": oss_full_path,
            #                 "extra": {
